                                  ...
                                  constant GLCloudUniforms& U [[buffer(1)]])
{
    ...
    // Distance LOD + external scalars (stepMul, quality). Quality lets us dial work without touching look params.
    ...
    int   N     = clamp(int(round(float(Nbase) * stepMul * quality)), 8, 24);
    float dt    = Lm / float(N);
    ...
    for (int i=0; i < N && T > half(0.004); ++i) {
        ...
        for (int k=0; k < refineMax && T > half(0.004); ++k){
            ...
            // Sun-only phase; premultiplied white
