...
struct CloudUniforms {
    float4 sunDirWorld;   // xyz = direction
    float4 sunTint;       // xyz = tint
    float  time;
    float2 wind;          // xy used
    float  baseY;
    float  topY;
...
// buffer(2) = CloudUniforms (bound from Swift via ofBufferNamed:"uniforms")
...
inline float  deg2rad(float deg)     { return deg * 0.017453292519943295f; }
...
// Analytic sun: bright disc + soft halo in EDR.
inline float3 sunGlow(float3 rd, float3 sunW, float3 sunTint)
{
    float ct   = clamp(dot(rd, sunW), -1.0f, 1.0f);
    float ang  = acos(ct);                   // radians from sun centre
    const float rad = deg2rad(0.95f);        // ~1° apparent size
    ...
    float edr  = core * 5.0f + halo1 * 0.90f + halo2 * 0.25f; // >1.0 on HDR
    return sunTint * edr;
}
                                ...
                                constant CloudUniforms& uniforms    [[buffer(2)]])
{
    ...
    baseCol += sunGlow(rd, sunW, uniforms.sunTint.xyz);

    // Intersect with [baseY, topY]
    float denom = rd.y;
    float tb = 0.0f, tt = -1.0f;
    bool hits = fabs(denom) >= 1e-4f;
    if (hits) {
        tb = (uniforms.baseY - ro.y) / denom;
        tt = (uniforms.topY  - ro.y) / denom;
    }
    float t0 = hits ? min(tb, tt) : 1e9f;
    ...
    if (hits && t1 > 0.0f) {
        ...
        float baseStep   = 220.0f * uniforms.stepMul;
    ...
    float3 outRGB = baseCol * trans + acc + uniforms.sunTint.xyz * acc * 0.08f;
