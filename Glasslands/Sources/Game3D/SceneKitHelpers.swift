...
enum SceneKitHelpers {
    ...
    @MainActor
    static func equirectSkyGradient(
        width: Int = 2048,
        height: Int = 1024,
        zenith: UIColor = UIColor(red: 0.50, green: 0.74, blue: 0.92, alpha: 1.0),
        horizon: UIColor = UIColor(red: 0.86, green: 0.93, blue: 0.98, alpha: 1.0)
    ) -> UIImage {
        let W = max(64, width), H = max(32, height)
        UIGraphicsBeginImageContextWithOptions(CGSize(width: W, height: H), false, 1)
        guard let ctx = UIGraphicsGetCurrentContext() else { return UIImage() }

        // Smooth vertical gradient (seam‑free equirect image).
        ...
        ctx.drawLinearGradient(
            grad,
            start: CGPoint(x: 0, y: 0),                  // top = zenith
            end: CGPoint(x: 0, y: CGFloat(H)),           // bottom = horizon
