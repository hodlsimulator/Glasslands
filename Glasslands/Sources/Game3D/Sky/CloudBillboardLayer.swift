...
//
//  Distribution:
//  • Ultra-far annulus: thin, hazy line near the horizon.
...
//  • Sprites have a hard transparent 2-px frame (see CloudSpriteTexture).
//
...
enum CloudBillboardLayer {
    ...
    static func makeAsync(
        ...
        let rFar1: Float = rFar0 + max(3000, Float(radius) * 1.40)           // “distance belt”
        ...
        let farC    = max(36, Int(Float(N) * 0.44))   // ~44%
        ...
        Task.detached(priority: .userInitiated) {
            ...
            let nearPts   = await CloudBillboardPlacement.poissonAnnulus(nearC,   r0: rNearHole, r1: rNearMax, minSepNear: 820, minSepFar: 980, seed: &s)
            let bridgePts = await CloudBillboardPlacement.poissonAnnulus(bridgeC, r0: rBridge0,  r1: rBridge1, minSepNear: 620, minSepFar: 780, seed: &s)
            let midPts    = await CloudBillboardPlacement.poissonAnnulus(midC,    r0: rMid0,    r1: rMid1,    minSepNear: 520, minSepFar: 700, seed: &s)
            let farPts    = await CloudBillboardPlacement.poissonAnnulus(farC,    r0: rFar0,    r1: rFar1,    minSepNear: 380, minSepFar: 560, seed: &s)
            let ultraPts  = await CloudBillboardPlacement.poissonAnnulus(ultraC,  r0: rUltra0,  r1: rUltra1,  minSepNear: 360, minSepFar: 520, seed: &s)
            ...
            for p in nearPts {
                specs.append(await CloudBillboardPlacement.buildCluster(
            ...
            for p in bridgePts {
                specs.append(await CloudBillboardPlacement.buildCluster(
                    at: p, baseY: layerY, bandSpan: bandSpan,
                    scaleMul: 0.98, opacityMul: 0.93, tint: simd_float3(0.98, 0.99, 1.00), seed: &s
                ))
            }
            for p in midPts {
                specs.append(await CloudBillboardPlacement.buildCluster(
                    at: p, baseY: layerY, bandSpan: bandSpan,
                    scaleMul: 1.00, opacityMul: 0.94, tint: simd_float3(0.97, 0.99, 1.00), seed: &s
                ))
            }
            for p in farPts {
                specs.append(await CloudBillboardPlacement.buildCluster(
                    at: p, baseY: layerY, bandSpan: bandSpan,
                    scaleMul: 0.88, opacityMul: 0.92, tint: simd_float3(0.95, 0.98, 1.00), seed: &s
                ))
            }
            for p in ultraPts {
                specs.append(await CloudBillboardPlacement.buildCluster(
                    ...
                    tint: simd_float3(0.90, 0.93, 1.00),
                    seed: &s
                ))
            }
