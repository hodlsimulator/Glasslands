...
// MARK: - File-private math (free functions so they can’t be @MainActor by inference)
...
// Builds cluster/puff specs off-main; pure value types only.
fileprivate func cbBuildSpecs(
    clusterCount: Int,
    minAltitudeY: Float,
    seed: UInt32
) -> [CloudBillboardLayer.Cluster] {
    @inline(__always)
    ...
    var clusters: [CloudBillboardLayer.Cluster] = []
    ...
    for centre in centres {
        ...
        var puffs: [CloudBillboardLayer.PuffSpec] = []
        ...
        for _ in 0..<count {
            ...
            puffs.append(CloudBillboardLayer.PuffSpec(
                dir: dir,
                size: max(8, size),
                roll: roll,
                atlasIndex: atlas
        ...
        clusters.append(CloudBillboardLayer.Cluster(puffs: puffs))
    }
