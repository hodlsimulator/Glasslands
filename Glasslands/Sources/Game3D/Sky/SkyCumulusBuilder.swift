...
private func rgba8ToCGImage(pixels: [UInt8], width: Int, height: Int) -> CGImage {
    let bpr = width * 4
...
/// Builds a bright white cumulus sky (2:1 equirectangular), tileable horizontally, no zenith void.
func buildCumulusCGImage(
    width: Int = 1536,
    height: Int = 768,
    coverage: Float = 0.34,
    ...
    // Cluster distribution: smaller/denser near horizon, larger up high, some at zenith.
    ...
    return rgba8ToCGImage(pixels: out, width: W, height: H)
}
