...
func renderCumulusPixels(
    ...
    @inline(__always) func clampf(_ x: Float, _ lo: Float, _ hi: Float) -> Float { x.isFinite ? min(hi, max(lo, x)) : lo }
    @inline(__always) func smooth01(_ x: Float) -> Float { let t = clampf(x, 0, 1); return t * t * (3 - 2 * t) }
    @inline(__always) func smoothstep(_ e0: Float, _ e1: Float, _ x: Float) -> Float { let d = e1 - e0; return d == 0 ? (x < e0 ? 0 : 1) : smooth01((x - e0) / d) }
    ...
    @inline(__always) func safeFloorInt(_ x: Float) -> Int { let y = floorf(x); return y >= Float(Int.max) ? Int.max : (y <= Float(Int.min) ? Int.min : Int(y)) }
    @inline(__always) func safeIndex(_ i: Int, _ lo: Int, _ hi: Int) -> Int { (i < lo) ? lo : (i > hi ? hi : i) }
    @inline(__always) func wrapIndex(_ i: Int, _ n: Int) -> Int { let m = i % n; return m < 0 ? m + n : m }
