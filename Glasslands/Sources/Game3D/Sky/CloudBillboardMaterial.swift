...
enum CloudBillboardMaterial {
    ...
    private static let marker = "/* VOL_IMPOSTOR_VSAFE_008_WHITE_ROUND_FAST */"

    @MainActor
    static func makeVolumetricImpostor() -> SCNMaterial {
        // Pure white puffs, round edges, single alpha sample, premultiplied.
        let fragment = """
        \(marker)
        #pragma transparent
        ...
        // Mild phase just to give sun-side brightness.
        ...
        // Alpha: ensure visibility even when T≈1 (no grey crush).
        float alphaOut = clamp(a0 * (0.18f + 0.82f * (1.0f - T)), 0.0f, 1.0f);

        // Colour: bright white with ambient lift; premultiplied.
        float ambient = 0.55f;
        float gain    = 2.20f;
        ...
        m.readsFromDepthBuffer = false   // sky layer: no depth test needed
