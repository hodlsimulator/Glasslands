...
//
//  Volumetric impostor for billboard puffs.
//  IMPORTANT: no ambient term; lit only by the sun (HG phase), tinted by sunTint.
//
...
enum CloudBillboardMaterial {
    ...
    static let volumetricMarker = "/* VOL_IMPOSTOR_VSAFE_085_MICRO_VAPOUR */"
    ...
    static func makeVolumetricImpostor() -> SCNMaterial {
        ...
        // Silhouette (analytic AA) and rounded-square guard
        ...
        // Lighting: sun only (no ambient), HG phase
        ...
        float ss = 0.0f;   // single-scatter “amount”

        // 10 slices (simple stratification)
        { float h=0.05f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.15f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.25f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.35f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.45f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.55f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.65f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.75f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.85f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }
        { float h=0.95f + jitter; float env=smoothstep(0.06f,0.40f,h)*(1.0f-smoothstep(0.58f,0.98f,h)); env*= (0.80f + gravK * pow(1.0f - h, 2.0f)); float dens = mask * env * 1.28f * vapourSoft; float a = exp(-kSigma*dens); ss += mask * (1.0f - a) * phaseHG; T *= a; }

        // Colour from the sun only (tinted)
        float3 C = clamp(ss, 0.0f, 1.0f) * sunTint;

        // Gentle lift at the horizon for readability, scaled by alpha
