...
//
//  Rayleigh + Mie sky via fragment shader-modifier (no binders).
//  Bright defaults; never writes depth; renders behind everything.
//
...
enum SkyAtmosphereMaterial {
    static func make() -> SCNMaterial {
        ...
        // Bright midday defaults so the sky cannot come out black
        m.setValue(SCNVector3(0, 1, 0),          forKey: "sunDirWorld")
        m.setValue(SCNVector3(1.0, 0.98, 0.95),  forKey: "sunTint")
        m.setValue(2.4 as CGFloat,               forKey: "turbidity")
        m.setValue(0.50 as CGFloat,              forKey: "mieG")
        m.setValue(1.9  as CGFloat,              forKey: "exposure")
