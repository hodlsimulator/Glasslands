...
//  Uses world position + sunDirWorld; no program binders; very fast.
//
...
enum SkyAtmosphereMaterial {
    static func make() -> SCNMaterial {
        ...
        float hr = mix(2.5, 0.8, elev);   // Rayleigh path
        float hm = mix(1.2, 0.3, elev);   // Mie path
