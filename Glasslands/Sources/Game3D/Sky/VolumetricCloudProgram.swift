...
// instead of many raymarched billboards. Uniforms are streamed from
// VolCloudUniformsStore into the shader buffer.
//
// If the Metal program cannot be constructed, this falls back to a lightweight
...
enum VolumetricCloudProgram {
    ...
    private static func makeFallbackMaterial() -> SCNMaterial {
        ...
        // Shader modifiers in this project use Metal-style sampling
        // (texture2d + sampler + .sample(...)).
        let fragLines: [String] = [
            ...
            "",
            "/* Cloud sampling: single equirect mask with rotation + drift. */",
            "float2 w    = wind.xy;",
            "float  wLen = length(w);",
            "float  spin = (time * 0.00035) * (0.25 + 0.75 * wLen);",
            "float3 Vr   = rotateY(V, spin + domainRotate);",
            "float2 uv   = equirectUV(Vr);",
            "uv += float2(w.x, -w.y) * (time * 0.000015)",
            "    + domainOffset.xy * 0.00002;",
            "uv.y = clamp(uv.y, 0.001, 0.999);",
            "",
            "/* Two taps from the same mask for richer silhouettes. */",
    ...
    private static func makeTrueVolumetricMaterial(program p: SCNProgram) -> SCNMaterial {
        ...
        m.lightingModel = .constant
        m.isDoubleSided = false
        m.cullMode = .front   // Opaque pass (shader returns alpha = 1).

        // Dome is an opaque background layer.
        m.blendMode = .replace
        m.transparencyMode = .aOne
        m.readsFromDepthBuffer = false
        m.writesToDepthBuffer  = false
        ...
        // No extra shader modifiers; SCNProgram drives this material.
        m.program = p
        return m
    }
}
