...
//  Adds a polar-only quality ramp (near zenith) to avoid GPU stalls while
//  keeping visuals unchanged for normal camera angles.
//
...
enum CloudImpostorProgram {
    ...
    static func makeMaterial(halfWidth: CGFloat, halfHeight: CGFloat) -> SCNMaterial {
        // Metal shader-modifier (fragment). Uses SceneKit’s Metal entrypoint with pragmas.
        let frag = """
        #pragma transparent
        ...
        float3 sunDirView;   // view-space sun dir (engine updates per frame)
        float   hgG;         // Henyey–Greenstein asymmetry
        float   baseWhite;
        float   lightGain;
        float   occK;
        ...
        // Macro 2D mask to break the circular card silhouette
        ...
        inline float sampleD3(float2 uvE, float z, float baseScale, float edgeMask, float coreFloorK) {
            ...
            // three gaussian-ish blobs for a soft core
        ...
        // Slab thickness and texture scales
        ...
        float3 camFwdW = normalize((scn_frame.inverseViewTransform * float4(0.0, 0.0, -1.0, 0.0)).xyz);
        // Dot with world-up: 1 = looking straight up
        float cosStart = 0.9659258; // cos(15°)
        float zt = clamp((dot(camFwdW, float3(0.0, 1.0, 0.0)) - cosStart) / (1.0 - cosStart), 0.0, 1.0);

        // Base integrator uses 5 fixed samples through the slab.
        // Near zenith, reduce to 3 samples max and relax the transmittance cutoff slightly.
        const int NMAX = 5;
        float ztSamples = mix(5.0, 3.0, zt);
        int   Ncalc     = int(round(ztSamples));
        float Tcut      = mix(0.008, 0.020, zt); // more aggressive early-out only near zenith

        // Equally spaced sample depths (midpoint rule around 0)
        float ztLUT[NMAX] = { -0.40, -0.20, 0.0, 0.20, 0.40 };
        ...
        for (int i = 0; i < NMAX && i < Ncalc && T > Tcut; ++i) {
            float d = sampleD3(uvE, ztLUT[i], baseScale, edgeMask, coreFloorK);
        ...
        // Material wiring (unchanged defaults)
        let m = SCNMaterial()
        m.lightingModel = .constant
        m.isDoubleSided = false
        m.cullMode = .back            // terrain occludes clouds at horizon
        ...
        // Tuned defaults (kept identical to existing behaviour)
