...
enum CloudImpostorProgram {
    ...
    static func makeMaterial(halfWidth: CGFloat, halfHeight: CGFloat) -> SCNMaterial {
        ...
        inline float fbm3_billow(float3 p){
            ...
            for(int i=0;i<4;++i){ float n=noise3(q); a += w * (1.0 - abs(n*2.0 - 1.0)); q *= 2.05; w *= 0.52; }
            return clamp(a,0.0,1.0);
        }
        inline float hg(float mu,float g){
            float g2=g*g; float denom=pow(max(1e-4,1.0+g2-2.0*g*mu),1.5);
        ...
        bool shapeValid = (shapeScale > 1e-6) && (shapeHi > shapeLo + 1e-6);
        float sMask = shapeValid ? macroMask2D(uvE*0.90, shapeScale, shapeLo, shapeHi, shapePow, shapeSeed) : 1.0;
        ...
        // Sun visibility (two taps)
        float3 Sdir = normalize(sunDirView + float3(1e-4,1e-4,1e-4));
        float2 sView = float2(Sdir.x,Sdir.y);
        ...
        float gain = clamp(baseWhite * lightGain * phase * Lvis, 0.0, 8.0);

        for (int i=0;i<N && T>0.004; ++i) {        // ← tiny perf guard; no visual change
            float d = sampleD3(uvE, zt[i], baseScale, edgeMask, coreFloorK);
            d = max(0.0, d + densBias);
            float aStep = 1.0 - exp(-sigmaS * d * dt);
            Cw += T * aStep * gain;
            T  *= (1.0 - aStep);
        }

        float alpha = clamp(1.0 - T, 0.0, 1.0);
        _output.color = float4(Cw, Cw, Cw, alpha);
        """
        ...
        // Safe defaults (engine overwrites every frame)
