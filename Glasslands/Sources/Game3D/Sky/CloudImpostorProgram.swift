...
//  Sun-only lighting (no self light). Premultiplied alpha; white albedo.
//
...
enum CloudImpostorProgram {
    ...
    static func makeMaterial(halfWidth: CGFloat, halfHeight: CGFloat) -> SCNMaterial {
        ...
        // No hard circles; noisy feathered edge
        float r = length(uvE);
        ...
        // Extra shape break-up so impostor outline doesn’t read as a disc
        float sMask = shapeMaskUV(uvE, shapeScale, shapeLo, shapeHi, shapeSeed);
        float edgeMask = interior * sMask;

        // Slightly fuller centres than rims
        ...
        float cosVS = clamp(-sView.z, -1.0, 1.0);   // view forward is -Z in view space
        float Fwd   = hg(cosVS, hgG);
        const float liftAtEdge = 1.0;               // edgeSoft deliberately unused

        for (int i=0; i < N && T > 0.003; ++i) {
            float z = -0.5*Lm + t;

            // Local density (with edge mask)
            float d = densityUV(uvE, z, covLocal, max(1e-4, puffScale), microAmp) * edgeMask;

            if (d > 0.0006) {
                // Tiny one-tap self-occlusion in sun direction
                ...
                // WHITE, sun-only: no base term, no rim lift; scale only by phase + shadow
        ...
        // ---- tuned defensive defaults (fast + bright, sun-only) ----
        m.setValue(0.70 as CGFloat,  forKey: "stepMul")      // ~14 steps
        ...
        m.setValue(3.80 as CGFloat,     forKey: "hiGain")    // stronger sun response
