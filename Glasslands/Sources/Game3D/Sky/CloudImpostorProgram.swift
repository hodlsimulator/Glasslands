...
enum CloudImpostorProgram {
    ...
    private static let fragmentSource: String = {
        ...
        float hash21(float2 p) { return fract(sin(dot(p, float2(12.9898, 78.233))) * 43758.5453123); }
        ...
        float densityAt(float3 pos, float cloudZ) {
            ...
            // Kept to preserve the current “puffy” contrast.
            // (This is still cheaper than extra ray steps.)
            dens = pow(dens, 1.35);
            return dens;
        }
        ...
        if (dot(p2, p2) > 1.0) { discard_fragment(); }

        float slabHalf = slab_half;

        // SceneKit shader modifiers: _surface.position is view-space.
