...
enum SkyMath {
    @inline(__always)
    static func clampf(_ x: Float, _ lo: Float, _ hi: Float) -> Float {
        x.isFinite ? min(hi, max(lo, x)) : lo
    }

    @inline(__always)
    static func mix3(_ a: simd_float3, _ b: simd_float3, _ t: Float) -> simd_float3 {
        a + (b - a) * t
    }
    ...
    @inline(__always)
    static func toByte(_ f: Float) -> UInt8 {
        UInt8(clampf(f, 0, 1) * 255.0 + 0.5)
    }

    @inline(__always)
    static func safeFloorInt(_ x: Float) -> Int {
    ...
    @inline(__always)
    static func safeIndex(_ i: Int, _ lo: Int, _ hi: Int) -> Int {
        (i < lo) ? lo : (i > hi ? hi : i)
    }

    /// Stable integer hash (used for per-pixel jitter).
    @inline(__always)
    static func h2(_ x: Int32, _ y: Int32, _ s: UInt32) -> UInt32 {
