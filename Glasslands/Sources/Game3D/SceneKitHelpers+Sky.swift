    ...
    static func skyWithCloudsImage(
        ...
        struct Perlin {
            ...
            static func noise(_ x: Float,_ y: Float,_ z: Float) -> Float {
                ...
                return lerp(w,
                    lerp(v, lerp(u, grad(p[AA],   xf,   yf,   zf),
            ...
            static func fbm(_ x: Float,_ y: Float,_ z: Float, octaves: Int = 5) -> Float {
                var amp: Float = 0.5, freq: Float = 1.0, sum: Float = 0.0, norm: Float = 0.0
                for _ in 0..<octaves { sum += amp * noise(x*freq, y*freq, z*freq); norm += amp; amp *= 0.5; freq *= 2.0 }
                return sum / max(0.0001, norm)   // ≈ [-1, +1]
            }
        }

        // Seed offsets so different seeds make different skies.
