...
enum SkyGen {
    static func skyWithCloudsImage(
        width: Int = 2048,
        height: Int = 1024,
        coverage: Float = 0.26,     // 0.22–0.32 for “fair-weather”
        thickness: Float = 0.30,    // lower = crisper puff edges
        seed: UInt32 = 424242,
        ...
        // sRGB avoids the “flat/grey” look we saw with DeviceRGB.
        ...
        // Low-res cloud field (accumulate elliptical puffs, then upsample)
        let gW = max(64, W / 3)
        let gH = max(32, H / 3)
        ...
        // Zenith cap prevents an overbright ring at the very top
        let capStart: Float = 0.975
        let capEnd: Float = 0.992

        // Scatter puffs (horizontally biased)
        for _ in 0..<count {
            let u = rng.unit()                  // [0,1)
            let v = rng.unit()                  // [0,1)
            let upY = cosf((v - 0.5) * .pi)     // 1 at zenith → -1 at nadir
            ...
            let maxJitter: Float = .pi * 0.055  // ±10°
            ...
            for gy in y0...y1 {
                ...
                let gravBias = 1.0 + 0.22 * smoothstep(0, sy * 0.8, max(0, yy)) // slightly fuller below centre
                for gx in x0...x1 {
                    ...
                    let d2 = xr*xr + yr*yr
                    let k: Float = 1.75
                    let shape = 1.0 / ((1.0 + k * d2) * (1.0 + k * d2)) // (1+k r^2)^-2
        ...
        // Normalise to 0…1 and gently raise contrast (≈ x^1.2)
        var fmax: Float = 0
        ...
        for i in 0..<field.count {
            ...
            x = clampf(x, 0, 1)
            field[i] = x * (0.8 + 0.2 * x)     // gentle gamma lift
        }
        ...
        for j in 0..<H {
            ...
            // Equirect → direction
            let az = (Float(j % 2 == 0) ? v : (1 - v)) // tiny stripe hack to avoid banding with the same RNG jitter per row
            _ = az // unused; RNG hack retained to keep parity with prior builds
            ...
            for i in 0..<W {
                ...
                // Clouds alpha from field + horizon/zenith shaping
                ...
                // Silver lining near the sun (kept subtle)
                ...
                // Cloud colour (lower base keeps sky blue visible)
