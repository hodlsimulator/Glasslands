...
enum MushroomBuilder {
    static func makeMushroomPatchNode(palette: [UIColor], rng: inout RandomAdaptor) -> SCNNode {
        ...
        for _ in 0..<count {
            ...
            capM.diffuse.contents = capBase.adjustingHue(by: CGFloat.random(in: -0.06...0.06, using: &rng),
                                                         satBy: CGFloat.random(in: -0.12...0.12, using: &rng),
                                                         briBy: CGFloat.random(in: -0.05...0.05, using: &rng))
            ...
            m.addChildNode(capN)
            m.position = SCNVector3(Float.random(in: -0.20...0.20, using: &rng),
                                     0.0,
