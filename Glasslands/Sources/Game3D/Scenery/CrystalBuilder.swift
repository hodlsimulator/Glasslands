...
enum CrystalBuilder {
    static func makeCrystalClusterNode(palette: [UIColor], rng: inout RandomAdaptor) -> SCNNode {
        ...
        for _ in 0..<count {
            ...
            let (shard, rad) = RockBuilder.makeRockNode(size: s, palette: palette, rng: &localRng)

            shard.scale = SCNVector3(0.9, Float.random(in: 1.6...2.4, using: &rng), 0.9)
            if let mat = shard.geometry?.firstMaterial {
                let tint = SceneryCommon.adjust(
                    baseTint,
                    dH: CGFloat.random(in: -0.05...0.05, using: &rng),
                    dS: 0.05,
                    dB: 0.02
                )
