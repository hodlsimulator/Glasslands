...
struct TerrainChunkNode {
    ...
    static func makeData(
        ...
        // Keep this as a thin wrapper: use the same math as ChunkMeshBuilder
        // so geometry and gameplay stay consistent.
        let builder = ChunkMeshBuilder(
            tilesX: cfg.tilesX,
            tilesZ: cfg.tilesZ,
        ...
        // Build synchronously on main (used only by warmupCenter)
        return awaitResult(builder.build(originChunkX: originChunkX, originChunkY: originChunkY))
    }
...
// Tiny helper to synchronously await an actor call on main (used only by makeData above).
@MainActor
private func awaitResult<T>(_ body: @Sendable @escaping () async -> T) -> T {
    var out: T!
    let g = DispatchGroup(); g.enter()
    Task { out = await body(); g.leave() }
    g.wait()
    return out
}
//  TerrainChunkNode.swift
